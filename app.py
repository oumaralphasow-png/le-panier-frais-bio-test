...
app = FastAPI(title="Le Panier Frais Bio API", version="45.0")

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # prototype uniquement
...
def health():
    ...
    return {"status": "ok", "service": "Le Panier Frais Bio API", "version":"45.0"}
    ...
    with db() as con:
        ...
        for lot in eligible:
            ...
            if take > 0:
                ...
                con.execute(
                    "INSERT INTO reservations(order_id,lot_id,quantity,status) VALUES(?,?,?,'ACTIVE')",
        ...
        if remaining > 0:
            raise HTTPException(
                409,
                detail={"code":"STOCK_INSUFFICIENT","requested":o.quantity,"missing":round(remaining,3)}
            )
        con.execute(
...
def cancel_order(order_id:int):
    with db() as con:
        con.execute("BEGIN IMMEDIATE")
        rows = con.execute(
            "SELECT id,lot_id,quantity FROM reservations WHERE order_id=? AND status='ACTIVE'",
            (order_id,)
        ).fetchall()
        if not rows:
